...
- WHT: ~~`wht_amount = total × rate/100`~~ — ⚠️ **WRONG, corrected by ADR 0016 Q6.** Thai withholding is computed on the amount **before VAT**: `wht_amount = subtotal_excl_vat × rate/100`. With `total` including VAT this over-withholds on every bill carrying both (10,000 + 7% at 3% → 300, not 321), and the 50 ทวิ certificate would not match what the recipient claims. `net_payment = total − wht` is unchanged.
...
> ⚠️ **ADR 0016 amends this table.** Not built: `allocation_method` (the two shared strategies need departments *and* revenue, neither reachable, leaving a column whose only value is MANUAL) · `bill_image_url` / `slip_image_url` (no object storage — ADR 0012 Q6's rule) · `payment_status = PARTIAL` (it describes an amount and there is no payments module to hold one; `UNPAID`/`PAID` ship). Added: `source_gr_id` is **unique** so a replayed GR confirm yields one expense, plus `recurring_expense_id` + `period` with a unique on the pair. `expense_item.department_id` is built as specced and stays nullable. See also the WHT correction above.
...
>
> ⚠️ **ADR 0017 settles `WASTE`: it is NOT a movement type.** Part 17 records waste as a `waste_log` document with `SourceType.WASTE_LOG`, posting an ordinary `ADJUST_LOSS` — the pattern ADR 0015 used for stock counts. Adding an enum value would mean dropping and re-declaring `stock_movement_sign_check` (Part 13's two-migration dance) plus every drift guard, to carry information the source type already carries. H.8's theoretical-vs-actual variance therefore reads `source_type`, not `movement_type`, when it lands in Sprint 5. **Par level** — named in the Sprint 3 plan line and specified nowhere else — is defined in ADR 0017 Q5/Q6: a `par_level` row per (product, branch), storing base units, driving a list and never an order.

> ⚠️ **ADR 0018 settles `TRANSFER`: it IS a movement type — four of them.** The single `TRANSFER` value above is superseded by `TRANSFER_OUT` / `TRANSFER_IN` / `TRANSFER_OUT_REVERSAL` / `TRANSFER_IN_REVERSAL` (Part 18). This is the opposite answer to ADR 0017's on waste, and deliberately so: FIFO must be able to cut the layer a transfer pushed rather than pop the head of the queue (ADR 0014 Q8), which only the movement type can express. One `stock_transfer_item` posts **two** movements — permitted because `unique(source_type, source_id)` is a key on the pair, and the two ends carry different source types (`TRANSFER_OUT` / `TRANSFER_IN`) over the same id; a third, `TRANSFER_SHORTAGE`, carries what was dispatched but never arrived. Stock in transit belongs to the **receiving** branch, and a transfer writes **no expense** — it is a move, not spend. **`INTERDEPARTMENT_TRANSFER` is untouched by Part 18** and remains Sprint 6+ work.
>
...
> ⚠️ **ADR 0019 supersedes this section — `sales_transaction` is NOT built as written.** §5.6 was designed from an imagined POS: one flat table carrying line-grain fields (`pos_menu_id`, `qty`, `unit_price`) beside bill-grain fields (`bill_subtotal`, `bill_vat`, `table_no`, `payment_method`), against the header+line shape every document Part since 11 has used. A real Thai POS export read during the Part 19 grill carries **no bill id, no time, no table, no staff and no channel** — one row is one menu summarised for one day — so most of those columns have no source to fill them.
>
> Part 19 ships **`sales_line`** at the grain **menu × sales day × branch**, with `bill_id`, `sold_at` and `channel` **nullable**, so a per-bill export and a daily-summary export land in the same table and share one code path. Supporting tables: `sales_import_profile` (column map + header signature + encoding + whether amounts already include VAT/service charge), `sales_day` (**branch × business_date**, which enforces that re-importing a day *replaces* it), `menu_category`, `menu_alias`. `pos_integration` keeps its shape **minus `api_credentials`** — no POS API is reachable by a Thai SME, so encrypted credentials would be a security surface with no writer.
>
> Revenue = **after discount, excluding VAT, excluding service charge**, normalised at import. Part 19 writes **nothing** to `stock_movement`: `CONSUMPTION` needs `recipe` and arrives in Sprint 5. The Section B three-layer mirror and `recipe_change_diff` also move to Sprint 5 — with no recipe, there is no enrichment to protect.
...
> ⚠️ **Three parts of this section are superseded by ADR 0032 (Part 32, 2026-09-01).**
> **(1) "Cost / Revenue / Gross Profit per cell" conflates two different costs.** A cell carries **materials cost only**; rent, electricity and other OpEx never enter a food-cost figure, and a shared expense (`ExpenseItem.department_id IS NULL`) stays in ส่วนกลาง and is never auto-allocated. OpEx by department is a separate report.
> **(2) "stock consumption/waste → `stock_movement.department_id`" describes a column that has never existed** and is not being added — `fifo-replay.ts` reads that table on every cost computation, and ADR 0011 made the ledger deliberately dumb. The department of consumed stock is **derived at read** from `menu.primary_department_id` by apportioning the real posted figure; waste and manual adjustments have no menu to trace back and therefore always sit in "ไม่ระบุแผนก".
> **(3) The default view permissions** (Owner/Manager full matrix, Branch Manager their column, Department Head their row, Kitchen Staff their cell) died with §H.4 — ADR 0029 deleted `ROLE_PERMISSIONS` and replaced roles-as-scopes with named capabilities plus branch reach.
> Still true and unchanged: the four aggregation slices, drill-down, the toggles, and cost attribution for **purchases** (`goods_receipt_item_allocation`) and **expenses**.
...
> ⚠️ **Superseded WHOLE by ADR 0029 (Part 28, built 2026-08-30). Nothing in this section was implemented, and five of its pieces were refused with reasons.** (1) The `role_permission` **table** is refused: capability names in rows are beyond the compiler's reach, where `cost:veiw` is a permission that silently disappears with nothing going red — and a table with no editing screen is a table nothing writes. The role→capability map lives in TypeScript (`src/lib/permissions/service.ts`). (2) **`role × resource_type × action` is refused as the unit.** Several things worth protecting are not resources at all: **cost is stored nowhere** (ADR 0014 — it is a ledger replay), approving a purchase order is not a table, inviting a person is not a table. A resource axis forces a fake resource named `"cost"` into existence, which is a lie in the shape of the data. The unit is a **named capability**; there are seventeen. (3) The **8-step algorithm** is refused: the check is one required argument on `requireTenant`, so a page that names no permission does not compile — the omission is a build error rather than a step somebody forgets. (4) **`scope_filter` dies in all three values.** `own_dept` — `department_id` reaches three business tables against `branch_id`'s fifty-eight and the ledger has none (ADR 0022 Q8), so the check would pass for almost every tenant and a gate that is always green is worse than none. `own_branch` — became `canAccessBranch` plus narrowing at the branch LIST, which is one rule instead of a filter per query. `own_record` — **uncomputable**: `stock_count`, `goods_receipt` and `sales_consumption_run` carry no authorship column at all, and `waste_log`, `stock_transfer` and `staff_meal` carry a free-text NAME that ADR 0028 Q4 deliberately kept from becoming an identity. (5) *"Test matrix: 50+ cases mandatory before Sprint 0 ends"* never happened; Sprint 0 closed 26 Parts earlier. What replaced it is in ADR 0029 Q15.
>
> The `Action` union and the interface below describe Drizzle, which this project does not use. **Read this section as history.** The rules that bind are `docs/calculation-rules.md` §11.8 (A1–A10).
...
> ⚠️ **Settled by ADR 0032 (Part 32, 2026-09-01) — and the premise below died before it was built.** This section was written when consumption was NOT in the ledger, so "theoretical" had to be computed separately. **Part 22 posts CONSUMPTION from recipes into `stock_movement`, so the ledger balance IS the theoretical balance**, and Part 15 already stores both sides of the comparison on `stock_count_item` (`qty_expected`, a snapshot at save, and `qty_counted`), with the variance computed at read. The SQL below also carries `department_id` on both CTEs — a column `stock_movement` has never had. What is actually missing is a **view**: the same variance per **product**, accumulated over a **period**, ranked by **money**. It is deliberately **not** split by department — nobody recorded whose hands the missing stock left, and apportioning it by usage share would print a guess where a reader expects a fact; the report names which departments use the product instead.
>
> *(Original note, kept for lineage:)* **Deferred to Sprint 6, and half-superseded.** The "actual" side must read `source_type`, not `movement_type` (ADR 0017), and its `st.qty` source table does not exist — the grain is `sales_line` (menu × day × branch). The internal contradiction is resolved in favour of the spec's own O16: **Sprint 6**, not Sprint 5 (decided 2026-08-21). It needs real data from both gross-profit methods to be worth reading anyway.
