...
| L3b | The GR-confirm/void hook + the Part 14 VAT uplift + `/cost` spend swap | ⏳ **blocked on one decision** — see "Open question" below |
...
### Open question (raised at the L3a/L3b boundary, 2026-08-17)
**Where does `goods_receipt.vat_rate_percent` come from?** ADR 0016 Q2 gives the receipt three VAT columns but never says who fills the rate in. Two readings, and the answer decides whether L5 adds a field to the receipt form:
- **A (recommended)** — a PO-based receipt **snapshots the order header's rate**; a standalone receipt prefills from the supplier's `defaultVatRatePercent` then the tenant default — and **the receiver may correct it**, because the GR is where *actuals* are recorded (`unit_price_actual` is already exactly this, ADR 0012 Q3) and the delivery's tax invoice is the authority, not the order. Cost: one more field on the receipt form at L5.
- **B** — inherit only, never editable. No form change, but a tax invoice whose VAT differs from the order cannot be recorded truthfully, and the FIFO layer would then be valued on the order's rate.

Either way `vat_amount` is **derived** (`subtotal × rate/100`), never typed — which keeps "the line's share of `vat_amount`" identical to `line_total_actual × rate/100` and lets the replay uplift each line without reading the receipt's other lines.
