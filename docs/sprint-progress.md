...
## Current Sprint: Sprint 7 — RLS · อีเมลขาออก · ต้นทุนตามแผนก 🚧 IN PROGRESS

**Status:** **Part 30 (เปิด RLS ให้ทำงานจริง) ✅ COMPLETE 2026-08-30** · **Part 31 (อีเมลขาออก) ✅ COMPLETE 2026-09-01** · **Part 32 (ต้นทุนตามแผนก + ของหายไปไหน) ✅ COMPLETE 2026-09-01** — Part 30 คือหนี้ที่ ADR 0004 เลื่อนมาตั้งแต่ Sprint 1 · Part 31 ปลด blocker ที่ทุกเอกสารเขียนผิดมาตั้งแต่ Sprint 4: มันเป็นสองการตัดสินใจ ไม่ใช่หนึ่ง และขาออกไม่เคยต้องรอขาเข้าเลย
...
## Sprint 7 Part 32 — ต้นทุนและรายได้ต่อแผนก + ของหายไปไหน: ✅ COMPLETE (L0–L6, 2026-09-01)

**ADR:** `docs/adr/0032-cost-and-revenue-by-department.md` (Q1–Q7, grill 2026-09-01)
**Rules:** `docs/calculation-rules.md` §11.10, **F1–F10**
**ไม่มี migration · ไม่แตะ ledger · ไม่แตะ Part 22**

Part นี้เริ่มจากที่ผู้เขียนเสนอให้ **ตัดมิติแผนกทิ้ง** แล้ว Kong ปฏิเสธอย่างถูกต้อง: *"ยังไงก็ต้องมีแผนก มันคือสิ่งที่เราต้องการตั้งแต่แรก — cost รายได้ตามแผนก ที่เราจะ diag ปัญหาของการจัดการร้านได้"* · การไล่ต่อจากคำปฏิเสธนั้นพบว่า**ช่องว่างมีจุดเดียว ไม่ใช่ทั้งระบบ**

- **สามในสี่เสามีอยู่แล้ว** — รายได้ต่อแผนก (`sales_line` → `menu.primaryDepartmentId` ซึ่ง `/menus` เตือนให้กรอกอยู่แล้ว) · ของที่ซื้อเข้าต่อแผนก (`GoodsReceiptItemAllocation`) · OpEx ต่อแผนก (`ExpenseItem`) · **ขาดแค่ต้นทุนของที่ถูกใช้ไป** เพราะ ADR 0022 ระเบิดเมนูทิ้งตอน post
- 🔴 **ทางที่เลือกไม่ต้องเก็บอะไรเพิ่มเลย (Q1)** — ปฏิเสธ `department_id` บน `stock_movement` (`fifo-replay.ts` อ่านตารางนั้นทุกครั้งที่คิดต้นทุน และ ADR 0011 ทำให้ ledger โง่โดยตั้งใจ — **แผนกคือ "ทำไม" และ "ทำไม" อยู่บนเอกสารเสมอ**) · ปฏิเสธคอลัมน์บน `SalesConsumptionItem` เพราะ **เกินจำเป็น ไม่ใช่เพราะผิด** หลัง Kong ชี้ว่าข้อมูลมีอยู่แล้ว · คำนวณตอนอ่านแบบเดียวกับที่ ADR 0014 ทำกับต้นทุนมา 31 Parts
- **กฎที่ทั้ง Part ยืนอยู่บนมัน (Q2, F2): ยอดรวมมาจาก ledger การแบ่งมาจากเมนู** — ถามยอดจริงจาก run ที่ยังยืนอยู่ → คิดสัดส่วนจากยอดขาย × สูตร → หารยอดจริง · **คอลัมน์แผนกบวกกันได้ COGS จริงโดยนิยาม** ไม่ใช่โดยบังเอิญ
- **สองตัวเลขที่ห้ามบวกกัน (Q3, F3)** — ของที่ซื้อเข้าให้แผนก ≠ ของที่แผนกใช้ไป · Kong อธิบายส่วนต่างครบกว่าที่ผู้เขียนเสนอ: ฿3,920 ทำให้ต้นทุน*เดือนนี้*ต่ำลง และ**เดือนหน้าต้องบวกกลับ** เพราะกินของที่เดือนนี้จ่ายไปแล้ว · สองวิธีคิด GP ต้องบรรจบกันเสมอ **และวันที่ไม่บรรจบคือวันที่มีของหาย**
- 🔴 **GP ต่อแผนกทำได้เฉพาะวิธีสูตรอาหาร (Q4, F4)** — วิธีนับสต๊อกซึ่งเป็นค่าเริ่มต้นของทุกร้านแยกไม่ได้ เพราะของที่เหลืออยู่ในห้องเก็บของของสาขา ไม่ได้อยู่ในแผนกไหน · **หน้าจอพูดออกมาเป็นกล่อง ไม่ซ่อน**
- **ค่าไฟค่าเช่าไม่เข้า COGS (Q5, F5)** — Kong ทักและถูก · `§Section F` ของ spec เขียนคำว่า "Cost" ต่อช่องแบบรวม ๆ **และตรงนั้น spec หลวม**

🔴 **§H.8 ตายไปครึ่งหนึ่ง และอีกครึ่งถูกสร้างไปแล้วโดยที่ spec ไม่รู้ตัว (Q6)** — spec เขียนตอนที่ consumption ยังไม่อยู่ใน ledger จึงต้องคำนวณ "ทฤษฎี" แยก **แต่ Part 22 ทำให้ยอดใน ledger *คือ* ยอดทฤษฎี** และ Part 15 เก็บอีกฝั่งไว้ตั้งแต่ Sprint 3 (`qty_expected` + `qty_counted`) · ที่ขาดคือ**มุมมอง**: รายวัตถุดิบ ต่อช่วงเวลา เรียงตามเงิน · SQL ใน spec ยังอ้าง `stock_movement.department_id` ที่ไม่เคยมีอยู่

🔴 **ส่วนต่างไม่ถูกหารเข้าแผนก และเป็นการปฏิเสธที่ตั้งใจ (Q7, F6)** — ไม่มีใครบันทึกว่าของหายจากมือใคร การหารด้วยสัดส่วนการใช้คือ**การกล่าวหาที่ข้อมูลไม่รองรับ** · รายงานบอกว่า**ใครใช้**วัตถุดิบตัวนั้นแทน แล้วให้เจ้าของตัดสินเองว่าจะถามใคร — **เป็นข้อมูล ไม่ใช่คำตัดสิน** · เทสต์ L4 ตรึงกฎนี้ไว้ที่**รูปร่างของ type** ไม่ใช่ที่คอมเมนต์: `LeakRow` ไม่มีที่ให้ใส่ส่วนต่างรายแผนก คนที่อยากหารต้องแก้ type ก่อน

**เรื่อง performance ที่วัดก่อนตัดสิน (ADR 0024 ห้ามเดาก่อนวัด):**

| วิธีเรียก `computeConsumptionForDayLogic` | ต่อวัน | 30 วัน 1 สาขา | 30 วัน 3 สาขา |
|---|---|---|---|
| แบบที่เทสต์เรียก (transaction ต่อครั้ง) | 149 ms | 4.5 วิ | 13.4 วิ |
| ในทรานแซกชันเดียว (แบบที่รายงานทำ) | **70 ms** | **2.1 วิ** | **6.3 วิ** |

ครึ่งหนึ่งของ 149 ms คือ overhead ของ `withTenantContext` ไม่ใช่ตัวงาน · **Kong เลือกทางที่เร็ว** → ยกส่วน fetch ออกจากลูประเบิด (`explodeMenuSalesLogic`) แล้วตัดช่วงตามวันที่ resolution เปลี่ยน · **ไม่ใช่การประมาณ**: ในหนึ่งช่วงทุกวัน resolve สูตรตัวเดียวกัน การระเบิดยอดรวมของช่วงจึงได้ผลเท่ากับระเบิดทีละวันแล้วบวก และปัดเศษครั้งเดียว · มีแค่สองอย่างที่ขยับขอบเขต ตรวจจากสคีมา: `recipe.effective_from` และ `menu_merge.effective_from` — **`recipe_branch` ไม่มีวันที่เลย** การรับสูตรมาใช้จึงมีผลกับทุกวันในอดีตพร้อมกัน (คุณสมบัติที่รับมา ไม่ได้สร้าง)

| L | What | ✅ |
|---|---|---|
| L0 | ADR 0032 · กฎ F1–F10 · แก้ `§Section F` + `§H.8` · CONTEXT.md 1 คำ | ✅ |
| L1 | `department-split.ts` — การหารที่เงินหายไม่ได้ · largest-remainder ยืมกติกา H.3 | ✅ |
| L2a | `byMenu` บน `ConsumptionDemand` + **วัดต้นทุนต่อวัน** | ✅ |
| L2b | ยก fetch ออกจากลูป · ตัดช่วงตาม resolution | ✅ |
| L3 | `departmentBreakdownLogic` + รายได้ต่อแผนก | ✅ |
| L4 | `/cost/departments` | ✅ |
| L5 | `/cost/leaks` (§H.8) | ✅ |
| L6 | เอกสาร · **ชิ้นที่สาม (dashboard) ถูกยกเลิกพร้อมเหตุผล** — ADR Consequence 9 | ✅ |

**Verified at close:** `tsc` clean · `build` green (**66 routes**, +2) · vitest **1302 passed / 4 skipped** · sweep เงียบ

⚠️ **ยังเหลือ:** `waste_log` ไม่มี `department_id` ของเสียจึงอยู่ "ไม่ระบุแผนก" เสมอ (F7) — เพิ่มได้ทีหลังในฐานะคอลัมน์บน**เอกสาร** ไม่ใช่บน ledger และ**จงใจไม่ทำจนกว่าจะมีร้านจริงบอกว่าเขาแยกของเสียตามแผนกจริง** · ตัวเลขสรุปบน dashboard ต้องมีที่เก็บผลคำนวณ ซึ่งขัด ADR 0014 — เป็นเรื่องของ Part อื่น
...
| L3a | `consumption.ts` — for one branch × day: resolve every sold menu's recipe **as of that day**, ONE `loadRecipeGraph` for all roots, `explodeToRaw` per root × servings sold, summed per product and **negated on the way out** (what the recipe consumed is what the ledger loses) · takes a `tx`, opens no context · the cancelled-sale policy parts **quantity** from **money**: revenue is what the till kept, whatever the policy says about stock · **a method-less PREPPED holds its whole dish back** — the walker emits it as a leaf, right for cost (UNPRICED) and wrong for stock, since nothing can raise a prepped balance · **18 DB tests, own tenant, buys nothing** | ✅ |
| L3b | `consumption-post.ts` — the run, its items and their movements, through `createStockMovementLogic` · **the run id is DERIVED** from `submitKey`+day (the `versionIdFor` device), because one press covers up to a month and only the first day could claim the key · a run is written **even when nothing posts**, so the screen can tell a day nobody tried from a day where every dish lacked a recipe · **the void lives here too, not in L3c** — Q2b makes re-posting and re-importing one mechanism, so splitting them would have written it twice · `assertSourceExists` gained its `SALES_CONSUMPTION` branch and **`isInboundType` gained `CONSUMPTION_REVERSAL`**, which the DB CHECK caught within a minute of the first positive item · **14 DB tests, own tenant, buys nothing** | ✅ |
| L3c | **`fifo-replay.ts` gains two cases — its first change since Part 18.** `CONSUMPTION` remembers what it took; `CONSUMPTION_REVERSAL` gives back exactly that, mirroring `TRANSFER_OUT_REVERSAL` including its accepted approximation (value exact, FIFO position at the back of the queue) · priced **`LAST_KNOWN`, not `DOCUMENT`** — the money is right but its provenance was mixed, and the walk does not track the mix · `stock-cost.ts` feeds the item's `reversal_of_item_id` in, the third batched lookup of that shape · **`commitSalesImportLogic` now voids a posted day automatically** and returns `consumptionRunsVoided`, but never re-posts · **8 DB tests, and this fixture BUYS** — two layers at two prices is the only shape where the two possible answers differ | ✅ |
...
One number per branch per day — what the customer paid — closing the window between periodic imports. Small on purpose: **no new tables**, five columns on `sales_day`, one enum with one value.
...
4. **There is no file storage anywhere in the project** — no dependency, no `.env` credential, nothing — and **two earlier Parts had already hit it and written the refusal into the schema** (`schema.prisma:1008` for a receipt's invoice photo, `:1190` for an expense's bill/slip). Finding the same sentence twice is what turned "attach a photo to the transfer" from a column on this table into **Feature 5**, a Part serving all four callers at once.
...
| **Q3** | 🔑 **Two kinds of driver, and only one will ever have a login** (sharpened by Kong after the first answer). A **company driver** is an employee and a tenant member — the end state is that they sign in and confirm the count themselves. A **hired outside driver** will never have a login, and giving one to a stranger is worse than no record; their evidence is a **photo of the handover**. Neither mechanism exists today (no user management, **no file storage at all**), so Part 18 ships `driver_user_id` (nullable FK) + `driver_name` + `driver_confirmed_at` + the qty the driver accepted, typed at A **in front of them** — the roadside count is what makes a shortfall fair to argue about; the login and the photo only make it harder to dispute. **`driver_user_id` is written now, `photo_url` deliberately is not**: the FK becomes fillable with no new infrastructure and no vendor decision, while the storage vendor decides whether a photo column holds a URL, a bucket key or a signed path. Attachments captured as **Feature 5** in `docs/pending-features-v1.5.md` — the **third** Part to want a photo (GR invoice since Part 13, expense slip since Part 16, both refused in `schema.prisma` for this same missing dependency) and a payment slip will want one next. *Rejected:* an account now — `canPerform` is dead code, so a driver login today writes stock and POs in every branch |
...
| L1 | Schema — `stock_transfer` + `stock_transfer_item` + 4 `MovementType` + 3 `SourceType` + `CostSource` + `StockTransferStatus` + 11 CHECKs + partial unique + RLS | ✅ **two migrations, both applied to Neon**: `20260818144758_part_18a_transfer_movement_types` (all four enum values in ONE file — the dance is per *migration*, not per value) then `20260818145200_part_18_stock_transfer` (tables + the DROP/re-declare of `stock_movement_sign_check`). ⚠️ **The generated migration had to be renamed**: `migrate dev` timestamps in **UTC** while the hand-made 18a folder used **local time**, so the generated `20260818074832…` sorted *before* 18a and a fresh `migrate deploy` would have hit the sign CHECK before the enum values existed. Renamed to `20260818145200…`; nothing was applied in the wrong order because 18a went first here. **Three decisions taken at the boundary:** (1) 🔑 **`cost_source` is frozen beside `cost_total`** (Kong's call) — money without provenance sends `0.00` to the receiving branch with no way to tell *"the sender never knew"* from *"these were free"*, and re-asking the sender later returns **today's** confidence, not the confidence at dispatch. First time `COST_SOURCE_VALUES` becomes a DB enum; an L2 test pins the two lists together. (2) **`qty_received` is NULLABLE, not defaulted to `qty_sent`** — the absence of a count is not agreement, and a default would let an unconfirmed transfer claim it arrived in full (`par_qty > 0`'s reasoning). (3) **Receiving MORE than was dispatched is refused by CHECK** — a shortfall has a posting, a cost and a name (`TRANSFER_SHORTAGE`); a surplus has none of the three, because no stock is created by driving a truck. Also `from_branch_id <> to_branch_id`: a mis-set dropdown would otherwise post an equal-and-opposite pair at one branch — balance right, history nonsense, and the ledger is append-only so it stays that way. **`tf_number` uses a FULL unique** (no soft delete on this table, so Pitfall #22/#23 cannot apply) unlike `sc_number`'s partial. `MOVEMENT_TYPE_VALUES` / `SOURCE_TYPE_VALUES` + Thai labels updated in the same layer, which puts the transfer values into `/stock/history`'s source filter for free — and **tsc caught the missing labels immediately**, which is what those `Record<>` types are for. Verified on Neon: 11 CHECKs · partial unique · RLS on both · 4+3 enum values · the sign CHECK naming all four · **0 tenants** |
| L2 | zod — dispatch, receive, void, queries | ✅ `src/lib/validations/transfer.ts`, **25 tests T1–T25**. 🔴 **T14 caught a real bug in the first draft, and it is the most dangerous kind this Part could ship:** `z.coerce.number()` reads `null` / `undefined` / `""` as **0**, and on the receive form **0 is a legal answer** ("nothing arrived") — so a blank box would have validated and written the whole line off as `TRANSFER_SHORTAGE`, **with a driver's name attached to a loss that never happened**. Every other quantity in the system is saved from this by accident, by a `.positive()` that happens to reject the coerced 0; `qtyReceived` is the first where 0 is meaningful, so the blank is now caught BEFORE coercion (`blankToUndefinedNumber` → `z.number({required_error})`), and unparseable text is left alone so `"abc"` still reports as the wrong *type* rather than as a missing value. **`dispatchedAt` is a true INSTANT**, not a date, for ADR 0013 Q4's reason plus one of its own: ADR 0014 Q9b resolves a date-only value to the END of its Bangkok day, which would put every dispatch after every receipt of the same day and draw the goods from the wrong FIFO layer. **No cost field crosses this boundary** (T3) — a cost from the browser is a number the sender could type rather than one their stock actually cost. `driverConfirmed` uses the `"true"/"on"` preprocess, not `z.coerce.boolean`, which reads the non-empty string `"false"` as **true** — a form posting that would silently claim the driver signed for the load (T10). Same-branch transfer is refused in zod as well as by CHECK, so the user gets a Thai sentence on `toBranchId` instead of a constraint violation (T2). **There is no third "quantity the driver accepted" column** — that number IS `qtySent`, typed at the sending branch in front of them; a second figure for one count would have nothing able to keep it honest, which is ADR 0017's argument against storing a base qty beside the ledger's. Also: a **compile-time drift guard** now pins `COST_SOURCE_VALUES` to the new Prisma enum (`stock-cost.ts`), backed by T25 at runtime — Part 18 turned that list into a DB enum, so a fifth value added in TS alone would surface months later as a constraint violation on one unlucky dispatch |
| L3a | Logic — **the cost walk learns transfers** (`fifo-replay.ts` + `stock-cost.ts` + the `/cost` split) · `stock-movement.ts` gains its sign and source branches | ✅ **The two layers swapped**, because the dependency runs the other way round: `transfer.ts` freezes the sending branch's FIFO money at dispatch, so the walk has to understand transfers first or the *second* transfer of any product prices itself wrong. **8 new cases F23–F30**, pure and DB-free, every one also asserting ADR 0014 Q12's money invariant. `CostMovement` gains `transferValue` + `transferPricing`: an arrival by transfer is the **only** inflow whose value comes from another branch's ledger, and falling through to the old `default` would have priced it at the RECEIVING branch's last known cost — **0 for a branch that never bought the product, i.e. goods arriving free** (F24). The sender's confidence travels too, so `0.00` reads as UNPRICED rather than as free (F25) and does **not** teach the receiver a last-known cost of zero. `TRANSFER_IN_REVERSAL` cuts **its own** layer via the same `reverseOwnLayer` helper `PO_RECEIVE_REVERSAL` now shares — "give back exactly what this document brought in" is one rule, not two (F27). `TRANSFER_OUT` stays in `default` and is correct there: it is an ordinary outflow at the sending end, and the money it draws IS what gets frozen onto the line (F26). **`/cost` needed only a one-line exclusion** (`out.type !== "ADJUST_LOSS"`) to keep both transfer legs out of ของเสีย *and* ส่วนต่าง — that cheapness is Q4's decision paying off, since a source-type-based design would have needed an exception per column. `TRANSFER_SHORTAGE` reaches ส่วนต่าง/ปรับปรุง deliberately: it is the only row there **with** a document, and it names a driver, which makes it the most actionable entry in the column rather than the odd one out. `assertSourceExists` gains the first source whose **branch depends on which source type is asking** — `TRANSFER_OUT` → sender, `TRANSFER_IN`/`TRANSFER_SHORTAGE` → receiver — and getting that wrong leniently would credit the goods back to the branch that gave them away, holding twice the stock that exists. `isInboundType` updated, or every `TRANSFER_IN` would be refused before the DB ever saw it |
| L3b | Logic — `src/server/transfer.ts` (dispatch posts both legs + freezes cost · receive posts the shortfall · void appends reversal lines) | ✅ **25 cases X1–X25** against real Neon. 🔑 **The frozen cost is obtained by ASKING THE ENGINE, not by re-deriving FIFO here**: the outflow is posted first, then `replayPairsInTx` is asked what that outflow cost, and the answer is written onto the line. There is therefore no second FIFO implementation to drift from the first, and `/cost` and the transfer document **cannot** disagree — they are the same number from the same function. `costSource` is read **before** the outflow posts, because it describes the goods about to leave and the front layer may be a different one the moment they have. X3 pins something the ADR did not spell out: a cost that came from a human declaration crosses the branch boundary **still labelled `DECLARED`** — travelling must not launder a guess into something that looks like it came off an invoice. Receiving refuses a **second, different count** (`TransferAlreadyReceivedError`): a genuine replay is already a no-op via `(TRANSFER_SHORTAGE, itemId)`, so a second answer is a *correction*, and corrections to a posted document are voids in this system (ADR 0011 Q7). The receive payload must answer **exactly** this document's lines — a partial answer would leave some lines confirmed and others NULL under a document saying RECEIVED, and nobody could later tell which a blank meant. The shortfall converts with the line's **frozen** ratio, so a sack resized mid-journey cannot change how much went missing on a trip that already happened. Voiding a **received** transfer also gives back the shortfall it recorded (X17), or the receiving branch keeps a loss for goods it is no longer deemed to have owned. ⚠️ **The teardown taught the schema back**: nulling `reversal_of_item_id` before deleting — the move `waste-logic.test.ts` uses — violates `stock_transfer_item_sign_check`, because a negative `qty_sent` is legal only ON a reversal line. Children are deleted before parents instead. **5 tenants stranded by that failure were swept on Kong's explicit go; Neon back to 0** |
...
| L3a | Logic — `src/server/waste.ts` (post + void through `createStockMovementLogic`) · `assertSourceExists` gains its `WASTE_LOG` branch | ✅ **13 cases L1–L13** against real Neon. The `WASTE_LOG` branch is the simplest of the four: unlike a GR line or a count line, a waste row carries **both** product and branch itself — it is a document of one row, with no header to inherit a place from. **A void is valued from the ORIGINAL MOVEMENT, not recomputed** from the input quantity and today's `toBaseRatio` (L13 is the test): a sack resized between the entry and its correction would otherwise credit back 60 kg for stock that left as 50, and the ledger would sit permanently richer than the shelf. Reversing means undoing what was posted, not posting what we would post today — ADR 0012 Q3's frozen-ratio instinct. The list hides a voided entry **and its reversal** by default, or every correction would count twice in a figure people read as a total. Waste may take the shelf negative (ADR 0011 Q9): a shop that never recorded the delivery still threw the tray away, and refusing the write would only lose the fact |
| L3b | Logic — `src/server/par-level.ts` (the three-state list **+ last-counted-at per product×branch**) | ✅ **15 cases R1–R15**. 🔑 **ADR 0017 Q6b corrected here (Kong's call):** freshness reads `stock_count_item.counted_at` on a **CLOSED count**, not the ledger's `STOCK_COUNT` movements as the ADR first said. `closeStockCountLogic` posts **nothing** for a zero-variance line, so the ledger cannot see a product that was counted and found exactly right — the best-managed stock in the shop would have read "ยังไม่เคยนับ" forever, i.e. the freshness line would have been most wrong about the products it had least to warn anyone about. R11 is that test. Still no new column: it reads the document instead of the movement. Setting a par is an **upsert that revives a soft-deleted row** rather than inserting beside it, so removing and re-adding does not grow a dead row per edit (R14). Outstanding-on-order uses the PO line's **frozen** `to_base_ratio` (ADR 0012 Q3) — what is owed is what was ordered — and two open orders are summed with the **earliest** expected date representing them, because the one already late is the one worth naming (R9). A missing expected date can never be overdue (R7, Consequence 7). Four reads, none per row |
| L4 | Actions + Thai errors + view serializers | ✅ `src/app/waste/{actions.ts,_components/waste-view.ts}` + `src/app/stock/{par-level-actions.ts,_components/par-level-view.ts}`. Thin glue, no rule decided here. Par's actions sit **beside** `/stock` in their own file rather than inside Part 10's `actions.ts` — the product page imports the same two, because a par is set from wherever the user already is (Consequence 4). **There are exactly two par actions**, and the absence of a third is the design: nothing here orders anything (Q5). `submit_key` is **read from the form, never minted server-side** — a minted key is a fresh key on every retry, and writing off the same tray of prawns twice is a real loss. Every waste write revalidates **`/cost`** too, which was not true of a spoilage adjustment before this Part: Q4 makes ของเสีย mean `WASTE_LOG` alone, so a waste entry now moves a column on the executive view. The freshness sentence ("นับล่าสุด 9 วันก่อน" / "ยังไม่เคยนับ") is rendered in the **serializer**, not the component — the usual hydration reason, plus one more: "how many days ago" depends on what *today* is, and today in Bangkok is not today in the browser's timezone |
| L5a | `/waste` — entry form + list + void + dashboard nav | ✅ A route of its own (Q7). **No draft, no confirm step, and the form stays open after a success** keeping product/branch/date — the kitchen throws away three things in a row, so `submit_key` **rotates** on each success or item #2 would read as a replay of item #1 and silently write nothing, which is the worst possible failure for a loss record. The negative-balance warning shows but **does not even offer a confirm checkbox**, unlike the adjust form: waste is not a correction the user might have got backwards, it is an event they watched happen (ADR 0011 Q9). Void is an inline `<details>` with a reason box, **not a `confirm()`** — a void needs a reason anyway, so a modal asking only "are you sure?" would be a second interruption for nothing, and a browser modal mid-Server-Action can wedge the page. The reason picker carries the **yield boundary** in one line of Thai (trim/cooking loss is yield; a staff meal is not waste), said where someone would otherwise get it wrong |
| L5b | par level on the product page · the below-par list on `/stock` | ✅ **Interactive cards**, per Kong's call — collapsed says *what to do* (product, gap, state) and expanded says *why* (the order behind it, the count date in full). One deviation, stated: **the freshness line stays visible while collapsed**, because Q6b calls it the row's own health warning rather than decoration, and a warning you must open a card to see is not a warning. The par setter lives on the **product page**, one row per branch including branches with **no** par — setting one must not require first knowing it is missing (Consequence 4). Each branch row is its own `<form>` with its own action state, or an error on ทองหล่อ would appear under อารีย์. **`/stock-counts/new` now honours `?branch=`** (Part 15's page + `OpenCountForm` gained `preselectBranchId`): the card links there for a named branch, and a link that silently ignored which branch it named would send the counter to the wrong shelf. The preselect wins even when that branch is already counting — the "already open" notice explains it better than landing them elsewhere silently would |
| L5c | `/cost` column rename + source split · remove `SPOILAGE`/`DAMAGE` from `/stock/adjust` | ✅ The split **inverted**: ของเสีย is now the narrow case (`WASTE_LOG` alone) and everything else that left without a document is **ส่วนต่าง/ปรับปรุง** (`STOCK_COUNT` + `ADJUSTMENT`). Written as an explicit `WASTE_LOG` check rather than an `else`, so a fifth source type lands in variance — where an undocumented outflow belongs — instead of silently inflating waste. `countVarianceValue` → **`varianceValue`**, since it is no longer only about counting. **`ADJUSTMENT_REASON_FORM_VALUES` = `RECOUNT` \| `OTHER`** drives the picker while `ADJUSTMENT_REASON_VALUES` keeps all four: zod still ACCEPTS the retired values, so a tab opened before the deploy does not fail on submit — and what it writes *is* an adjustment, which is true and lands in variance by the same rule as everything else. **K9b** is the new test that pins it: same product, same day, 10 kg with a waste document → ของเสีย 200, 5 kg typed by hand → ส่วนต่าง 100. K9 and K11's assertions moved columns rather than being deleted — that move IS Consequence 1 (the past is relabelled by a rule, not by a migration). Both screens say out loud where the old spoilage adjustments went |
| — | **UX pass, after the function is complete** | ✅ 2026-08-18. Kong's sequencing, and it earned its place — **two of the four findings were function bugs, not taste.** (1) `getWasteLogsLogic` had **no `take` at all**: a kitchen logging ten things a day writes ~3,650 rows a year, every one of them on a single page, forever. Now capped at `MAX_WASTE_ROWS = 200` fetched as +1 so the page can say *"showing the first 200"* rather than ending mid-history. (2) `/stock` rendered **nothing** when the below-par list was empty, which collapsed two very different situations — *nobody has set a par* (the exact failure Consequence 4 predicted: the feature stays invisible to the shop that never used it) and *everything is above par* (a shop doing fine, denied the one useful thing an empty list can say). Three states now. (3) The list **defaults to the current month** instead of all history — the period a shop actually reviews, and the same cycle the par design rests on; both ends are shown in the form, so the narrowing is visible rather than silent. Date + product filters exposed (the schema already had them), with **L14** covering the inclusive day boundary, because an off-by-one there would hide *today's* entries — the ones someone is most likely looking for. (4) `/stock` gained a **บันทึกของเสีย** button beside ปรับสต๊อก: that is where someone stands when they notice stock is wrong, and the adjust form no longer offers ของเสีย as a reason. Row scale is shown as a **count, with the baht linked to `/cost`** — summing quantities across products would invent a number that means nothing ("3 กระสอบ + 5 kg" is not 8 of anything) |
| L6 | Throwaway E2E + verify + push | ✅ **13 cases E1–E13** through the real action stack (FormData → zod → `toFormError` → `*Logic` → DB CHECK), spec + dedicated config **deleted** — never committed. A double POST with one `submit_key` writes **one** entry · `2 กระสอบ` leaves as 50 kg · a qty rounding to 0 comes back as a Thai field error naming **both** units instead of a constraint violation · the retired `SPOILAGE` is refused **on the waste form** even when posted by hand · voiding credits the stock back exactly and the second void is refused in Thai · a par of 0 is refused on its field while deleting the row removes the product from the list · an open PO turns ต้องสั่ง into สั่งแล้ว **without** suppressing the row · a **zero-variance count still sets the freshness** (E11 — the Q6b correction, end to end) · `/cost` splits by source · waste at อารีย์ leaves ทองหล่อ's par row alone |

### Verified (L6, 2026-08-17)
- `pnpm tsc --noEmit` clean · `pnpm build` green (**42 routes** — `/waste` is the new one) · `pnpm vitest run` **501 passed / 4 skipped** (453 Part-16 baseline + 19 L2 + 13 L3a + 15 L3b + 1 new cost case).
- **Part 14's own suite re-run, not just the new ones**: `getBranchCostSummaryLogic` changed which column an outflow lands in, so K9 and K11 moved with it and K9b was added.
- **Neon swept: 0 tenants.**

### Part 17 — ✅ COMPLETE (L0–L6 + the UX pass, 2026-08-18)

A kitchen can now say **"we threw this away"** and a manager can see **"this is about to run out"** — and the two numbers that result mean different things on purpose. `/cost`'s ของเสีย column finally means food someone watched go in the bin, with a date and a person to talk to; everything that left without a document sits beside it in ส่วนต่าง/ปรับปรุง, which is a different conversation with a different manager. Nothing was migrated to make that true: the past is relabelled by a rule.

**Carried forward from this Part:**
- **The UX pass found two function bugs, not four cosmetic ones** — an unbounded list query and an empty state that hid the feature from the shops that had never used it. Worth remembering the next time a pass like this looks skippable.
- **Par ships before the input that makes it good, and says so on screen.** Until H.5 (Sprint 4 `sales_transaction` + Sprint 5 `recipe`), nothing deducts what was sold, so the ledger balance only rises between counts. Every par row therefore carries how fresh its figure is, and the design survives H.5 unchanged — consumption only improves the input.
- **`ตามของ` depends on expected delivery dates**, which are optional on a PO. A shop that never fills them in gets the two-state list, and the card says that rather than leaving the third state mysteriously unreachable.
- **`SPOILAGE`/`DAMAGE` are still valid enum values** and zod still accepts them; only the form stopped offering them. A stale tab writes an adjustment, which is true.
- **A fourth writer now goes through `createStockMovementLogic`** (receipts, adjustments, counts, waste). Part 18's transfer should not be the first exception.

**Part 14 was touched again** — `getBranchCostSummaryLogic` re-splits its outflows (`WASTE_LOG` vs `STOCK_COUNT` + `ADJUSTMENT`) and `countVarianceValue` became `varianceValue`; its suite was re-run, not just the new ones.
