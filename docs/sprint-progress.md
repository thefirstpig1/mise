...
### Pre-existing defect found at L5a (NOT Part 10 — needs a decision)
`pnpm build` fails type-checking on `src/app/login/page.tsx` (Sprint 0, `6a2db97`): Next 15 made `searchParams` a **Promise**, and the page still types it as a plain object. `next build` prints **✓ Compiled successfully** first — every route including the new `/stock` ones compiles — then dies in the type-check pass. `pnpm tsc --noEmit` does NOT catch it (the generated `.next/types/**` route types are outside the tsconfig scope). Effect: production builds are blocked repo-wide; dev + tests are unaffected, which is why 9 parts went by without noticing. One-line fix (`searchParams: Promise<{...}>` + `await`), deliberately NOT bundled into the Part 10 batch.
