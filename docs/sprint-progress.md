...
| L3a-2 | Read logic — `getProductCostLogic` (branchId required, `asOf?`), the **batch** primitive taking `productIds[]` (R1), `getBranchCostSummaryLogic` | ⏳ The GR_LINE source projection in `stock-movement.ts` gains `lineTotalActual` + `reversalOfItemId` — additive, no schema change |
