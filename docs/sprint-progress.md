...
| **Q6b** | **Every row states how fresh its number is.** There is **no stock prediction in this system** — and until Sprint 4–5 the ledger balance only rises, because nothing deducts what was sold (`MovementType` has no `CONSUMPTION`; H.5 needs `sales_transaction` + `recipe`). A par alert on that number alone would go silent in exactly the shops that need it. So each row shows when a physical count last confirmed it, below-par **and** stale sorts to the top, and the list doubles as the prompt to go and count. **Survives H.5 unchanged** — consumption improves the input, and freshness matters *more* once the number moves by itself |
