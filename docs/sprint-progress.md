...
## Current Sprint: Sprint 7 — RLS · อีเมลขาออก · ต้นทุนตามแผนก · ธีม · deploy 🚧 IN PROGRESS

**Status:** **Part 30 (เปิด RLS ให้ทำงานจริง) ✅ COMPLETE 2026-08-30** · **Part 31 (อีเมลขาออก) ✅ COMPLETE 2026-09-01** · **Part 32 (ต้นทุนตามแผนก + ของหายไปไหน) ✅ COMPLETE 2026-09-01** · **Part 33 (ธีม + โลโก้) ✅ COMPLETE 2026-09-02** · **Part 34 (ที่อยู่ของโปรดักชัน) 🚧 IN PROGRESS — หมุด A งาน 1–5 เสร็จ 2026-09-03** — Part 30 คือหนี้ที่ ADR 0004 เลื่อนมาตั้งแต่ Sprint 1 · Part 31 ปลด blocker ที่ทุกเอกสารเขียนผิดมาตั้งแต่ Sprint 4: มันเป็นสองการตัดสินใจ ไม่ใช่หนึ่ง และขาออกไม่เคยต้องรอขาเข้าเลย · Part 34 ปลด blocker ที่ค้างมาหลายเดือนด้วยวิธีเดียวกัน: **ทำให้การเลือกย้อนกลับได้ แทนที่จะพยายามเลือกให้ถูก**
...
## Sprint 7 Part 34 — ที่อยู่ของโปรดักชัน: 🚧 IN PROGRESS (หมุด A งาน 1–5 ✅ 2026-09-03)

**ADR:** `docs/adr/0033-deployment-host.md` (Q1–Q13, grill 2026-09-03)
**ไม่มี migration · ไม่มี dependency ใหม่ · ไม่แตะ ledger**

การเลือก host ถูกบันทึกว่าเป็น blocker มาตั้งแต่ปิด Sprint 7 และบล็อกทุกอย่างที่เหลือ: rate limit ชั้น 2 ออกแบบไม่ได้จนกว่าจะรู้ว่า edge เป็นของใคร · `AUTH_URL` ยังเป็น `localhost` · คำเชิญของ Part 29 จึงยังใช้ไม่ได้แม้เขียนเสร็จไปแล้ว

🔴 **คุณค่าหลักของ ADR นี้คือทำให้การเลือกย้อนกลับได้ ไม่ใช่การเลือก Fly** — เรื่องนี้ค้างเพราะกลัวเลือกผิด พอ build เป็น container มาตรฐาน การย้ายเจ้าคือเปลี่ยนปลายทางที่ push · **ตัดสินใจผิดแล้วแก้ได้ในหนึ่งชั่วโมง แปลว่ามันไม่ใช่การตัดสินใจที่น่ากลัวอีกต่อไป** — รูปแบบเดียวกับที่ ADR 0031 ใช้หด "เลือก vendor อีเมล" ให้เหลือแค่แก้ hostname ใน `.env`

- **สิงคโปร์เป็นเงื่อนไขคัดตัวเลือก ไม่ใช่ข้อดี (Q1)** — ทุกการอ่านเปิด interactive transaction (`runTenantTransaction` ถูกเรียกจาก **174 จุด**) หนึ่งหน้าจึงใช้ ~15 round trip · สิงคโปร์ ~40 ms/หน้า · สหรัฐฯ ตะวันออก **~3.5 วินาที/หน้า** · **ค่า default ของ Vercel คือ `iad1`** และผลลัพธ์จะดูเหมือนโค้ดห่วยทั้งที่เป็นระยะทางล้วน ๆ
- **เซิร์ฟเวอร์ Node เดียวรันค้าง ไม่ใช่ serverless (Q2)** — "เปิด transaction ต่อการอ่านหนึ่งครั้ง" คือรูปแบบที่ serverless + pgbouncer รับได้แย่ที่สุดพอดี และโปรดักชันไม่มี singleton ของ Prisma (`db.ts:167` ตั้ง global เฉพาะเมื่อไม่ใช่ production) ทุก cold start จึงเป็น pool ใหม่ = เครื่องผลิต connection เย็น ซึ่ง ADR 0024 วัดไว้ว่าค้าง ~0.3% และค้างถาวร · ระบบหลังร้านมีคนใช้ไม่กี่คน **ถ้า scale-to-zero เกือบทุกครั้งที่เปิดหน้าจะเป็น cold start**
- **Dockerfile เขียนเอง (Q3)** — ปิดสองรูที่ grill เจอด้วย: **ไม่มี `postinstall` และไม่มีสคริปต์ `prisma generate`** (ตอนนี้อาศัย `allowBuilds` ให้ postinstall ของ `@prisma/client` รันเอง — host ที่ใช้ pnpm คนละเวอร์ชันจะได้ client เก่าเงียบ ๆ) และ **ไม่มี `engines` ล็อก Node** · **Debian ไม่ใช่ Alpine** เพราะ Prisma ส่ง engine คนละไฟล์ต่อ libc และผิดตัวจะพัง **ตอน runtime ไม่ใช่ตอน build** · `node:*-slim` ไม่มี `openssl` ซึ่ง engine ต้องใช้ — พังตอน runtime เหมือนกันและอ่านเหมือนปัญหาฐานข้อมูล

🔴 **ADR เขียนผิดตรงจุดที่รับน้ำหนัก และเจอตอนลงมือ (Q6)** — ADR เขียนว่า "18 ใน 19 ไฟล์ idempotent มีแค่ `enforce_rls.sql` ที่ไม่ใช่" · ของจริง: **มี 18 ไฟล์ และรันซ้ำไม่ได้ *สอง* ไฟล์** — `enable_rls.sql` มี `CREATE POLICY` **47 อัน** และเขียนบอกตัวเองไว้ในหัวไฟล์ว่า *"APPEND-ONLY ไม่ idempotent"* มาตั้งแต่ Sprint 2

- release command ที่รันทุก deploy **มีไฟล์ที่พังครั้งที่สองไม่ได้** — deploy จะถูกยกเลิกทุกครั้งโดยที่ร้านไม่เห็นอะไรผิด (เวอร์ชันเก่ายังวิ่งอยู่) → `DROP POLICY IF EXISTS` นำหน้าทั้ง 47 อัน
- **ทำไมมันปลอดภัยทั้งที่ `enforce_rls.sql` เขียนคัดค้านวิธีนี้ไว้เอง** — ไฟล์นั้นใช้ `ALTER POLICY` โดยให้เหตุผลว่า policy ที่ถูก drop คือตารางที่ไม่มี policy · เหตุผลถูก แต่ข้อสรุปไม่ข้ามมาที่นี่: `ENABLE ROW LEVEL SECURITY` อยู่บรรทัดบนของทุก policy และ **ตารางที่เปิด RLS แต่ไม่มี policy = ปฏิเสธทุกแถวสำหรับ role ที่ไม่ใช่เจ้าของ** ช่องว่างจึง fail-closed
- และมันไม่มีช่องว่างเลยด้วยซ้ำ: **Prisma ส่งทั้งไฟล์เป็นคำสั่งเดียว** Postgres จึงห่อเป็น implicit transaction — **แต่ละไฟล์ลงทั้งหมดหรือไม่ลงเลย**

🔴 **`output: 'standalone'` เปิดตลอดไม่ได้ เพราะวินโดวส์** — Next สร้าง symlink ตาม pnpm store เพื่อประกอบโฟลเดอร์นั้น และการสร้าง symlink บนวินโดวส์ต้อง Developer Mode → `EPERM` **หลังจาก compile และ type-check ผ่านไปแล้ว** ซึ่งอ่านเหมือนโค้ดพัง · ปล่อยไว้เท่ากับ **ยึด `pnpm build` ไปจากทุก Part ต่อไปบนเครื่องนี้** (CLAUDE.md บังคับไว้ เพราะ tsc มองไม่เห็น route types) เพื่อแลกไฟล์ที่มีแต่ Dockerfile อ่าน → กั้นที่ `BUILD_STANDALONE=1`

🔴 **`.dockerignore` ที่ตัด `tests/` ออกทำ build พัง และมันสอนกฎหนึ่งข้อ** — ตัดออกดูเหมือนฟรี เพราะโปรดักชันไม่รันเทสต์ · แต่ `next build` type-check ทุกอย่างที่ tsconfig รวมไว้ ซึ่งคือ `**/*.ts` ซึ่งคือ `scripts/sweep-test-tenants.ts` ซึ่ง `import` จาก `../tests/support/sweep` · build ตายที่ type-check ด้วย module-not-found ของไฟล์ที่มีอยู่จริงในรีโป · **ตัด source ออกจาก build context เพื่อประหยัดไม่กี่ร้อย KB ไม่ได้อะไรเลย และจ่ายด้วย failure ที่อ่านเหมือน import พัง**

**Q6 + Q9 คู่กันคือสิ่งที่แก้หนี้ที่ค้างสองสปรินต์** — สคริปต์อย่างเดียวยังต้องมีคนรัน · `release_command` อย่างเดียวยังต้องมีคนบำรุงรายชื่อไฟล์ · **สองอย่างคู่กันทำให้ลืมไม่ได้เชิงโครงสร้าง**: `tests/manual-sql-coverage.test.ts` แดงเมื่อมีไฟล์ในโฟลเดอร์ที่ไม่อยู่ในลำดับ และ Fly ยกเลิก deploy เมื่อสคริปต์ exit ไม่เป็นศูนย์ · **ปฏิเสธ:** checklist ในเอกสาร — เรามีของแบบนั้นแล้วในรูป "⚠️ ยังเหลือ" **และมันค้างมาสองสปรินต์ ซึ่งเป็นหลักฐานว่ารูปแบบนี้ไม่ทำงาน**

**`enforce_rls.sql` เลิกมี `'REPLACE_ME_BEFORE_RUNNING'`** — ไฟล์ที่ต้องให้คนแก้ก่อนรันคือไฟล์ที่ถูกข้าม และมันถูกข้ามบนเส้นทางโปรดักชันมาสองสปรินต์ · รหัสถูกขยายจาก `MISE_APP_DB_PASSWORD` **ในหน่วยความจำ** แล้ว pipe เข้า `prisma db execute --stdin` — ไม่แตะดิสก์ ไม่เข้า Docker layer · `CREATE ROLE` ย้ายเข้า `DO $` ที่เช็ค `pg_roles` ส่วน `ALTER ROLE ... PASSWORD` อยู่ข้างนอกและรันทุกครั้ง **ซึ่งทำให้การ rotate รหัสกลายเป็น "เปลี่ยน secret แล้ว deploy"** — ปิดหนี้สุดท้ายของ ADR 0030 · `ALTER DEFAULT PRIVILEGES` เลิกฮาร์ดโค้ด `neondb_owner` แล้วถาม `current_user` เพราะ Q8 สร้างโปรดักชันเป็น**โปรเจกต์ใหม่** และ default privilege ที่ผูกกับ role ผิดจะไม่ให้สิทธิ์อะไรเลยโดยไม่บ่น

**สคริปต์ปฏิเสธสามอย่างแทนที่จะ escape** — placeholder ที่ชื่อตัวแปรไม่อยู่ใน allowlist · ตัวแปรที่ไม่ได้ตั้ง (**ขยายเป็นค่าว่างจะตั้งรหัสเปล่าแล้วรายงานว่าสำเร็จ ซึ่งแย่กว่าพังทุกแบบ**) · ค่าที่หลุดชุดอักขระแคบ ๆ · และปฏิเสธ deploy ที่รหัสใน `DATABASE_URL` ไม่ตรงกับ `MISE_APP_DB_PASSWORD` — **สอง secret ที่ต้องตรงกัน และเวลาไม่ตรงจะไม่มีอะไรบอกจนกว่าจะเปิดหน้าแรกของเวอร์ชันที่ขึ้นไปแล้ว**

**rate limit ชั้น 2 (Q7)** — ชั้น 1 เขียนไว้เองว่าไม่จับ **หมื่นที่อยู่ยิงคนละครั้ง** ซึ่งคือรูปร่างของการโจมตี deliverability พอดี และ deliverability คือสิ่งที่เจ็บจริง เพราะ magic link เป็นทางเข้าเดียว — **ผู้โจมตีไม่เดือดร้อน ร้านที่จ่ายเงินเดือดร้อน**

- นับในหน่วยความจำ**ถูกต้องเพราะ Q2 กับ Q4 ตัดเงื่อนไขที่จะทำให้มันเป็น hack ทิ้งไปแล้ว** — เป็นข้อสรุป ไม่ใช่ทางลัด
- อ่าน **`Fly-Client-IP`** ที่ proxy ตั้งและไคลเอนต์ปลอมไม่ได้ · **ไม่เคยอ่าน `X-Forwarded-For`** ซึ่งปลอมได้ = **ผู้โจมตีเลือก key ของ rate limit เอง** · ไม่มี header ให้ **ผ่าน** ไม่ใช่บล็อก (dev ไม่มี edge)
- 🔴 **cap ของ Map ไม่ใช่การปัดเศษ** — Map ที่ไม่มีขอบเขตคือ DoS แบบเดียวกันยกระดับหนึ่ง: ล้าน source address โตจนเครื่อง 1GB ถูก OOM kill ซึ่ง **Q13 บอกว่าหน้าตาเหมือน "แอปพังสุ่ม ๆ"** · evict ตัวที่เห็นล่าสุดนานที่สุด และ **fail OPEN สำหรับคนที่ถูก evict** (เลิกถูกนับ ไม่ใช่ถูกปฏิเสธ) ชั้น 1 ยังยืนอยู่ข้างล่าง
- 🔴 **และ cap เวอร์ชันแรกเองก็เป็นบั๊ก ที่เทสต์จับได้เพราะมันใช้เวลา 4.5 วินาที** — sweep ทันทีที่เกิน cap ทำให้ map เหลือเท่า cap พอดี insert ถัดไปจึงเกินอีก = **sort หมื่นรายการทุก request** · **ตัวป้องกันเผา CPU มากกว่าตัวโจมตี พอดีในเคสที่มันเกิดมาเพื่อรับ** → sweep เมื่อเกิน cap 10%: **9,096 ms → 52 ms**
- `src/instrumentation.ts` **พูดข้อสมมุติออกมาตอนสตาร์ท** พร้อมชื่อ machine id เพราะวันที่มีสองอินสแตนซ์ ลิมิตจะกลายเป็น 40/ชม. เงียบ ๆ

**สามหน้าที่ไม่เคยมี (Q12)** — **`loading.tsx` ไม่ใช่งานเสริม มันคือเงื่อนไขที่ทำให้ Q8 ยอมรับได้**: Q8 ยอมให้ Neon หลับโดยมีเหตุผลรองรับว่า "จะให้หน้าจอบอกว่ากำลังโหลด" **แต่โปรเจกต์ไม่มีไฟล์นั้นสักไฟล์ คำสัญญานั้นจึงไม่มีของรองรับ** — หน้าแรกของทุกเช้าและหลังพักเที่ยงค้างขาว 1–3 วินาทีเงียบ ๆ และร้านอ่านว่าแอปพัง · **localhost ไม่มีวันแสดงทั้งสามหน้าให้เห็น เพราะ dev โชว์ stack trace overlay และ 404 ภาษาอังกฤษของ Next** — กับดักแบบเดียวกับที่ ADR 0031 เจอตอนเปิดส่งอีเมลจริง · `error.tsx` โชว์ `digest` แต่**ปิดข้อความ** เพราะ error จาก Server Component พก table name / เศษ SQL มาได้ และ Next ตัดทิ้งในโปรดักชันด้วยเหตุผลนั้น — **`digest` คือเส้นเดียวที่ต่อสิ่งที่คนอ่านเห็นเข้ากับสิ่งที่ `fly logs` บันทึก และเป็นของที่ใกล้ error tracking ที่สุดจนกว่าหมุด B** · **ไม่มี `global-error.tsx` โดยตั้งใจ** — root layout โหลดฟอนต์แล้ว render children ไม่มี data fetch จึง throw ไม่ได้ และหน้า error ที่ไปถึงไม่ได้คือของอีกชิ้นที่ต้องคอยดูแลให้เข้ากับธีมโดยเปล่าประโยชน์

| หมุด A | งาน | ใคร | ✅ |
|---|---|---|---|
| 1 | `Dockerfile` + `.dockerignore` + `output: 'standalone'` (กั้น) + `engines` | ผม | ✅ |
| 2 | `scripts/apply-manual-sql.mjs` + `manual-sql-order.mjs` + เทสต์ M1–M6 | ผม | ✅ |
| 3 | `enable_rls.sql` + `enforce_rls.sql` รันซ้ำได้ · รหัสจาก env | ผม | ✅ |
| 4 | rate limit ชั้น 2 (`Fly-Client-IP`, 20/ชม.) + `instrumentation.ts` | ผม | ✅ |
| 5 | `error.tsx` · `not-found.tsx` · `loading.tsx` | ผม | ✅ |
| 5.5 | `system_reference_seed.sql` — ข้อมูลอ้างอิงกลางเข้าโปรดักชันได้ | ผม | ✅ |
| 6 | Neon **โปรเจกต์ใหม่** + บัญชี Fly | Kong | ⬜ |
...
🔴 **ฐานข้อมูลใหม่จะไม่มี `unit_template`/`liquid_density_template` เลย และเจอตอนเขียน `fly.toml`** — มันเป็นข้อมูล **global ไม่ใช่ per-tenant** `tenant-init.ts` จึงไม่สร้างให้ และ `prisma migrate deploy` สร้างแต่ตาราง · **ร้านแรกกดเพิ่มสินค้าแล้วช่องหน่วยว่างเปล่า** ซึ่งคือสิ่งแรกที่ทุกร้านทำหลังสมัคร และมัน**พังเงียบ**: ไม่มี error มีแต่ dropdown ที่ไม่มีตัวเลือก · เดิมอยู่ใน `prisma/seed-system.ts` ที่รันด้วย `tsx` ซึ่งเป็น devDependency **จึงเรียกจาก release command ไม่ได้** · Kong เลือกทาง A: ย้ายเป็น `prisma/manual/system_reference_seed.sql` ซึ่ง **ไม่สร้างกลไกใหม่เลย** — เข้าลำดับที่ release command รันทุก deploy อยู่แล้ว และเทสต์ `M1` คุมไม่ให้ตกหล่น · ปฏิเสธทางที่ต่อท้าย `pnpm release` ด้วย `.mjs` เพราะ**เพิ่มขั้นที่ไม่มีอะไรคุม** = เปิดช่องให้ลืมช่องใหม่ ทั้งที่ Q6 เพิ่งปิดช่องแบบนั้นไป · `pnpm db:seed:system` ชี้มาที่ `--only` ซึ่งเป็น **filter บนลำดับเดิม ไม่ใช่ทางเข้าที่สอง** (การ shell ออกไปเรียก `prisma db execute` ตรง ๆ จะเป็นโค้ดเส้นที่สองที่ drift ทันทีที่ฝั่งใดฝั่งหนึ่งเปลี่ยน) · `seed-system.ts` ถูกลบ ข้อมูลมีที่อยู่เดียว

🔴 **และเคสที่สำคัญที่สุดคือเคสที่รันบน dev แล้วไม่โดน** — dev มีข้อมูลอยู่แล้ว การรันจึงพิสูจน์แค่ทาง UPDATE ส่วนทาง **INSERT ซึ่งคือฐานข้อมูลใหม่วันแรก** ไม่เคยถูกแตะ · ทดสอบแยกในทรานแซกชันที่ล้างสองตารางให้ว่าง รันไฟล์ ตรวจได้ 11/5 ถูกต้องทุกคอลัมน์ รันซ้ำอีกรอบยังได้ 11/5 แล้ว **roll back** — dev ไม่ถูกแตะเลย

**เทสต์สองชุด พิสูจน์ว่าแดงได้จริงก่อนเชื่อเขียว** (ADR 0024 Consequence 3) — M1 แดงเมื่อวางไฟล์ที่ไม่อยู่ในลำดับ · M5 แดงเมื่อถอด `DROP POLICY IF EXISTS` ออกหนึ่งอัน · ทั้งคู่กลับเขียวหลังคืนค่า

**Verified (งาน 1–5):** `tsc` clean · `build` green (**67 routes**) · vitest **1329 passed / 4 skipped** (+18) · sweep เงียบ

⚠️ **ยังเหลือ — คู่มือละเอียดอยู่ที่ `docs/launch-3-tasks.md` ("3 Task ก่อน Launch")**
- **งาน 6–8 ต้องใช้บัญชี** — Neon **โปรเจกต์ใหม่ ไม่ใช่ branch** (Q8: branch คัดลอกข้อมูลจากพ่อแม่ โปรดักชันจะได้เศษข้อมูลเทสต์ติดมา · และรัศมีความเสียหายสำคัญกว่า — `db:reset --force` กับ `test:sweep` เขียนว่า "dev branch only" · **คนละ branch ในโปรเจกต์เดียว = ชี้ผิดคือพิมพ์ผิดตัวเดียว · คนละโปรเจกต์ = ต้องไปเอา credential คนละชุดมาวางเอง ซึ่งเกิดโดยบังเอิญไม่ได้**)
- 🛑 **`.env` ต้องเพิ่ม `MISE_APP_DB_PASSWORD`** ให้ตรงกับรหัสใน `DATABASE_URL` เป๊ะ (ยังไม่แตะ — `.env` เป็น stop-and-ask)
- **health check ต้องเป็น TCP เท่านั้น (Q13)** — ที่คิวรีฐานข้อมูลจะทำให้ compute ไม่มีวันหลับ = เผาโควตาแพลนฟรี 24 ชม. **ทำลาย Q8 เงียบ ๆ**
- **หมุด B ก่อนร้านแรก:** ซื้อโดเมน + SPF/DKIM + เปลี่ยน `AUTH_URL` · **`pg_dump` ตั้งเวลาเป็น GitHub Action** (Neon ฟรีเก็บประวัติ **6 ชั่วโมง**) · **error tracking** (`fly logs` ดูได้แค่ของสด ร้านเจอปัญหาตอนเช้าแล้วบอกตอนเย็น log อาจหายแล้ว)
- **ยังไม่ได้ตรวจ บันทึกไว้แทนที่จะเดา:** โควตาชั่วโมง compute ของ Neon แพลนฟรีเทียบการใช้จริง (ต้องมีร้านจริงใช้งาน)

### วัดของจริงก่อน deploy (2026-09-04) — ปิดสองข้อที่ ADR เขียนเองว่ายังไม่ได้ตรวจ

**ทำไมต้อง seed ก่อน:** dev Neon มี **0 tenant** — sweep ของ test suite ลบทุก tenant ทิ้งตอนจบรัน ตามที่ ADR 0023 Q4 ออกแบบไว้ ถ้าวัดตอนนั้นทุกหน้าจะคืน "ไม่มีข้อมูล" แล้วจบ และ FIFO replay จะไม่ได้ replay อะไรเลย จึงสร้างร้านหนึ่งเดือนขึ้นมาก่อน ผ่าน **logic function ตัวจริง** ทุกจุดที่มีเอกสาร (สินค้า, ใบรับของ, สูตร, การโพสต์ consumption) — เพราะ ledger ที่ยัดมือเข้าไปคือการวัดรูปร่างที่แอปไม่เคยผลิต: **1,013 stock_movement · 750 sales_line · 192 GR item · 24 expense · 30 consumption run** (1 สาขา, 30 วัตถุดิบ, 30 เมนู, 26 สูตร)

**Q13 — หน่วยความจำบนเครื่อง 1GB: พอ อย่างมีที่เหลือมาก** (`--memory=1g --cpus=1`, image `mise:part34`)

| | หน่วยความจำ | CPU |
|---|---|---|
| idle | 131–153 MiB (13–15%) | 0% |
| 8 ตัวยิงพร้อมกัน | peak **188 MiB** (18%) | ~35% |
| 24 ตัวยิงพร้อมกัน | peak **238 MiB** (23%) | **90–105% (ตัน)** |
| หลังหยุดยิง | กลับมา 147 MiB | 0% |

`OOMKilled=false`, `RestartCount=0` ตลอด · 🔴 **สิ่งที่วัดแล้วกลับไม่ใช่อย่างที่ `fly.toml` เขียนไว้: query engine ของ Prisma 5.22 เป็น library ที่โหลดอยู่ในโปรเซสเดียวกัน ไม่ใช่โปรเซสแยก** (`/proc` ในคอนเทนเนอร์มี pid เดียวคือ `next-server`) — คอมเมนต์ที่ว่า "หน่วยความจำของ engine ไปวางทับของ Next server" จึงไม่จริง 1GB ยังเป็นตัวเลือกที่ดี (ส่วนต่าง ~$2.6/เดือน ซื้อ headroom 4 เท่า) แต่**เหตุผลที่เขียนกำกับไว้ผิด** และของจริงบอกว่า 512MB ก็อยู่ได้ที่ ~46% · **ตัวที่ตันก่อนคือ CPU ไม่ใช่ RAM** — 24 request พร้อมกันกิน 1 vCPU หมด ขณะที่ `[http_service.concurrency] hard_limit = 200` ยังไม่ได้วัดและอยู่สูงกว่านั้นมาก

**Q1 — "~15 round trip, ~40 ms ต่อหน้า": เลขคูณที่ไม่เคยวัด ตอนนี้วัดแล้ว และมันแม่นสำหรับหน้ากลาง ๆ**

วัด RTT จาก**ในคอนเทนเนอร์**ถึง Neon สิงคโปร์ (เครื่อง Kong ที่กรุงเทพ) แล้วพิสูจน์ว่าโมเดลเป็นเชิงเส้นจริง ไม่ใช่สมมุติเอา:

- `select 1` เปล่า ๆ n=40 → **p50 33.3 ms** (min 32.7 / max 34.5) = หนึ่ง round trip
- transaction (BEGIN + `SET LOCAL` + 1 query + COMMIT) n=15 → **p50 133.7 ms = 4 × 33.3 พอดี** — ยืนยันประโยคใน CLAUDE.md ว่า "สี่ round trip ต่อหนึ่งการอ่าน" ด้วยการวัด
- transaction เดิมแต่ 5 query → p50 266.8 ms · **ต้นทุนส่วนเพิ่มของ query ที่ต่อคิวเพิ่มอีก 4 ตัว = 133.1 ms → 33.3 ms ต่อ round trip เป๊ะ**

โมเดลจึงเป็น `เวลาต่อหน้า = 7 ms + N × RTT` โดย 7 ms คือ `/login` (หน้าที่ไม่แตะฐานข้อมูลเลย) และ **N หาได้จากการกลับสมการที่พิสูจน์แล้วว่าเป็นเชิงเส้น ไม่ใช่การเดา**

| หน้า | วัดจริงที่ 33.3 ms (p50) | N (round trip) | คาดที่ `sin` 1 ms | ที่ 3 ms | ถ้าไปอยู่ `iad1` 230 ms |
|---|---|---|---|---|---|
| `/login` (ไม่แตะ DB) | 7 ms | 0 | 7 ms | 7 ms | 7 ms |
| `/products` | 320 ms | 9.4 | 16 ms | 35 ms | 2.2 s |
| `/cost/leaks` | 458 ms | 13.5 | 21 ms | 48 ms | 3.1 s |
| `/consumption` | 484 ms | 14.3 | 21 ms | 50 ms | 3.3 s |
| `/goods-receipts` | 513 ms | 15.2 | 22 ms | 53 ms | 3.5 s |
| `/sales` | 525 ms | 15.6 | 23 ms | 54 ms | 3.6 s |
| `/menus/coverage` | 544 ms | 16.1 | 23 ms | 55 ms | 3.7 s |
| `/dashboard` | 728 ms | 21.7 | 29 ms | 72 ms | 5.0 s |
| `/cost` | 924 ms | 27.5 | 34 ms | 90 ms | 6.3 s |
| `/stock` | 993 ms | 29.6 | 37 ms | 96 ms | 6.8 s |
| `/recipes` | 1,252 ms | 37.4 | 44 ms | 119 ms | 8.6 s |
| `/cost/departments` | 1,317 ms | 39.3 | **46 ms** | **125 ms** | **9.0 s** |

**หน้ากลาง = 16.1 round trip เทียบกับที่ Q1 เดาไว้ 15 · และ 3.7 วินาทีที่ `iad1` เทียบกับที่ Q1 เขียนไว้ ~3.5 วินาที** — เลขคูณของ Q1 แม่นอย่างน่าประหลาดสำหรับหน้ากลาง ๆ **สิ่งที่มันประเมินต่ำไปคือหน้าที่หนักที่สุด** ซึ่งใช้ 39 round trip ไม่ใช่ 15 → **9.0 วินาที ไม่ใช่ 3.5** การเลือก `sin` จึงยิ่งถูกกว่าที่ ADR อ้าง ไม่ใช่น้อยกว่า

⚠️ **ขอบเขตของตัวเลขชุดนี้:** จำนวน round trip **วัดแล้ว** · RTT 1–3 ms ของ `sin`↔`sin` **ยังเป็นสมมุติฐานของ ADR 0033 วัดไม่ได้จนกว่าจะมีบัญชี Fly** — ช่องสองคอลัมน์ขวาจึงเป็นการฉาย ไม่ใช่การวัด · และการทดสอบทั้งหมดยิงจากเครื่องเดียวกับคอนเทนเนอร์ ระยะทางที่วัดได้จึงเป็นของ **แอป→ฐานข้อมูล** ล้วน ๆ ซึ่งเป็นพจน์ที่ใหญ่ที่สุดพอดี

---

## Sprint 7 Part 33 — ธีมและโลโก้: ✅ COMPLETE (2026-09-02)

**ไม่มี ADR** — เป็นงานออกแบบที่ตัดสินด้วยการทำของจริงมาเทียบสี่รอบ · 8 commits, 120 files, +1,113/−704 · ไฟล์ใหม่สองไฟล์: `src/app/icon.svg`, `src/components/layout/Logo.tsx`

**ครีม · ข้าวสาลี · เสจ · โอลีฟเข้ม** — `#F8F3E1` `#E3DBBB` `#AEB784` `#41431B` · ทั้งสี่ค่ามีหน้าที่ ไม่มีอันไหนเป็นของประดับ: ครีมคือพื้นที่จม ข้าวสาลีคือเส้นแบ่งและแถบเน้น เสจคือไฮไลต์โปร่ง โอลีฟเข้มคือแบรนด์ · **หมึกต้องเพิ่มเข้ามา** เพราะไม่มีสีไหนในสี่สีเข้มพอจะวางตารางเงินได้

- **ทำไมอุ่นไม่ใช่น้ำเงิน** (เทียบคู่แข่ง 2026-09-02) — MarketMan เป็น teal + น้ำเงินบนขาว ซึ่งคือหน้าตา default ของ SaaS ทุกเจ้าบนโลก และ Wongnai คือ `#0070A8` · ธีมเดิมของไฟล์นี้ยืนอยู่ตรงนั้นพอดี · FoodStory ครองส้มจัด · **โทนอุ่นที่ desaturate แล้วใช้ทั้งพื้นผิว ไม่ใช่ใช้เป็น accent เดียว คือช่องที่ไม่มีใครยืนอยู่**
- 🔴 **`layout.tsx` เคยมี "รู" ไม่ใช่ "สไตล์"** — `tailwind.config.ts` อ้าง `var(--font-sans)` มาตั้งแต่ Sprint 0 **และไม่เคยมีใครนิยามมัน** ทุกหน้าจึงตกไปที่ `system-ui`: Tahoma บนวินโดวส์ Thonburi บน iOS อย่างอื่นบนแอนดรอยด์ — **สามผลิตภัณฑ์ตามอุปกรณ์ และไม่เคยมีใครเลือกสักอัน**
- **สองฟอนต์ สองหน้าที่** — Prompt รับหัวเรื่องกับตัวเลข (ตัวเลขถูกวาด ไม่ใช่ยืมมา ซึ่งสำคัญบนหน้าที่เป็นเงินเกือบทั้งหน้า) · Noto Sans Thai รับเนื้อความกับตาราง เพราะแคบกว่า แถวที่อัดแน่นจึงยังพอดีในขนาดที่คนอ่านออก · ใช้ Prompt ทั้งคู่จะเสียอย่างใดอย่างหนึ่ง **และหลังร้านจ่ายไม่ได้ทั้งสองอย่าง**
- **ที่ไหนได้ฟอนต์ไหนอยู่ใน `globals.css` สามบรรทัด** ไม่ใช่ class บนทุกหัวเรื่อง — `h1`–`h4` และอะไรก็ตามที่มี `tabular-nums` (103 จุด) ได้ display face · `tabular-nums` เป็นตะขอที่ถูกเพราะมันมาร์กสิ่งที่เราหมายถึงอยู่แล้ว
- **ไม่มี token `info` และไม่มีสีน้ำเงิน โดยตั้งใจ** — sky/blue ที่เหลืออยู่ 11 จุดกลายเป็น **สถานะกลาง ไม่ใช่คำตัดสิน** ทั้งหมด ("ราคาเฉพาะสาขานี้", "สร้างอัตโนมัติจากใบรับของ") และสองจุดในนั้นชิปข้าง ๆ เป็น `muted` อยู่แล้ว · **มันเป็นน้ำเงินเพราะไม่มีที่อื่นให้ไป ไม่ใช่เพราะน้ำเงินแปลว่าอะไร**
- **`good`/`bad` แปลว่าดีหรือแย่ต่อร้าน ไม่ใช่ขึ้นหรือลง** — Mise โชว์ต้นทุนกับรายได้บนจอเดียวกัน รายได้ขึ้นคือดี ต้นทุนขึ้นคือแย่ **สีที่ผูกกับทิศทางจะแปลว่าคนละอย่างในสองตารางที่อยู่ติดกัน**
- 🔴 **`Logo.tsx` เป็นคอมโพเนนต์ไม่ใช่ไฟล์ static ด้วยเหตุผลเดียว** — ร่องบนเมล็ดข้าวกว้าง ~1.9 หน่วย ต่ำกว่า 28px มันบางกว่าหนึ่ง device pixel จึงถูก**ตัดทิ้ง** เหลือวงรีเปล่า **ซึ่งคือวงบนของเครื่องหมายเปอร์เซ็นต์พอดี** — มาร์กจึงไม่เสียตอนย่อ **มันกลายเป็นอีกความหมายที่จริงของตัวเอง** · น้ำหนักเส้น **เพิ่มขึ้น**เมื่อมาร์กเล็กลง (`weightFor`) · **อย่า "ทำให้ง่ายขึ้น" เป็นไฟล์ SVG**

**Verified at merge:** `tsc` clean · `build` green (**67 routes** — 66 + `/icon.svg` ที่ Next สร้างให้) · vitest **1311 passed / 4 skipped** · sweep เงียบ

⚠️ **ยังเหลือ (เล็ก ไม่บล็อก):** `.btn`/`.badge` เป็นคลาสแคปซูลร่วม (radius 8px ยังกระจายอยู่) · ตัว `i` ของคำว่า Mise เป็นเมล็ด (ต้องวาด letterform เป็น path ก่อน) · palette ดิบเหลือ 64 จุด (emerald/orange/rose) — **คนละงานกับ sky/blue ซึ่งกวาดจนเหลือ 0 แล้ว** · **ไม่มี commit `docs(part-33): L6` โดยตั้งใจ** เพราะตามธรรมเนียมรีโปนี้ commit นั้นแปลว่า Part จบ และมันยังไม่จบ
...
⚠️ **ยังเหลือ:** **B** ซื้อโดเมน + verify SPF/DKIM (ตอนนี้ส่งถึงได้เฉพาะอีเมลเจ้าของบัญชี Resend) · **C** deploy + `AUTH_URL` จริง — **จนกว่าจะถึง C คำเชิญของ Part 29 ยังใช้จริงไม่ได้ เพราะลิงก์ยังชี้ `localhost`**; **C คือ Part 34** · ~~**rate limit ชั้น 2 (ต่อ IP) ต้องมาก่อน B**~~ → **ทำแล้วใน Part 34** (`src/lib/email/ip-rate-limit.ts`, ADR 0033 Q7) · `.env` มี `EMAIL_FROM` สองบรรทัด บรรทัดแรกเป็นค่าเก่าจาก Sprint 0 ที่ dotenv ทับให้อยู่แล้ว แต่ลบทิ้งจะปลอดภัยกว่า
