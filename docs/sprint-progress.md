...
| L | Layer | Note |
|---|---|---|
| **L0** | Docs — **ADR 0014** (Q1–Q12 + the risk register + the rejected alternatives) · CONTEXT.md cost entries rewritten (`Recipe cost`'s *"uses latest product_cost_history"* is now wrong twice over — no such table, and "latest" meant last purchase price) · master-spec §5.7 superseded note, the way §5.5 got one for ADR 0011 · this section | |
| L1 | Schema — `stock_cost_declaration` + RLS + **partial unique `(movement_id) WHERE superseded_at IS NULL`** (one live declaration per movement; a full unique would be Pitfall #22/#23 again) + CHECK `unit_cost >= 0` | Only new table in the Part; the ledger is untouched |
| L2 | zod — declaration input (+ the optional cost on the adjust form, in the unit the user typed) | |
| L3a | **Read logic — the replay engine.** `replayFifoLayers(movements, openingStack = [])` pure + tested standalone; `getProductCostLogic` (branchId required, `asOf?`); the **batch** primitive taking `productIds[]` (R1); `getBranchCostSummaryLogic` | The GR_LINE source projection in `stock-movement.ts` gains `lineTotalActual` + `reversalOfItemId` — additive, no schema change |
| L3b | Write logic — declare / supersede, in the adjust transaction or standalone | |
| L4 | Actions + Thai errors + view serializer (every Decimal → string) | |
| L5a | Per-product cost page (layers · `costSource` · declare + history) | |
