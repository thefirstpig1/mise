...
- 🔴 **ข้อมูลอ้างอิงกลางยังไม่มีทางเข้าโปรดักชัน** — ฐานข้อมูลใหม่ได้ตารางและ constraint ครบ แต่ไม่ได้ `unit_template`/`liquid_density_template` ซึ่งอยู่ใน `prisma/seed-system.ts` และเป็นข้อมูล **global ไม่ใช่ per-tenant** `tenant-init.ts` จึงไม่สร้างให้ → **ร้านแรกกดเพิ่มสินค้าแล้วช่องหน่วยว่างเปล่า** · **Kong เลือกทาง A แล้ว (2026-09-03) ยังไม่ลงมือ**: ทำเป็น `prisma/manual/system_reference_seed.sql` แล้วชี้ `db:seed:system` มาที่ไฟล์นั้น — **ไม่สร้างกลไกใหม่เลย** เข้าลำดับที่มีอยู่ ถูก release command รันทุก deploy และเทสต์ `M1` คุมอยู่แล้ว · ทางที่ปฏิเสธคือต่อท้าย `pnpm release` ด้วย `.mjs` ซึ่ง**เพิ่มขั้นที่ไม่มีอะไรคุม** = เปิดช่องให้ลืมช่องใหม่ ทั้งที่ Q6 เพิ่งปิดช่องแบบนั้นไป
