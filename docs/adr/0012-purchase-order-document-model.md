...
> **Wording reconciled (Part 13.5, 2026-08-16).** This ADR says the line is *"copied onto the line at send time"*; the implementation writes the snapshot on **every `DRAFT` save** and freezes it by immutability at `SENT` (`src/server/purchase-order.ts`). The guarantee is identical — a `DRAFT` is not a document anyone else holds, so re-snapshotting it is not a change of anything — but "at send time" reads as though a draft carries live FKs until the moment it is sent, and it never does. Read "frozen at send time" as *"frozen **by** send"* throughout this ADR, including its title.
