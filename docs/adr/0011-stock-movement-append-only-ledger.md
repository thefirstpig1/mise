...
>
> **Part 13.5 clarification (2026-08-16) — the guarantee only holds if the CALLER supplies the source id.** Q4 is keyed on `(source_type, source_id)`, so a producer that mints a *fresh* source id per call never presents the same key twice and the pre-insert lookup can never fire: the replay writes a second source row and a second movement, and the stock doubles. Part 10's `createStockAdjustmentLogic` did exactly that until now — the idempotency was real at the movement layer and absent at the layer above it, which is the only layer a double POST reaches. **Both producers now take a client-minted `submitKey` uuid and use it AS the source row's primary key** (Part 13 for `goods_receipt`, Part 13.5 for `stock_adjustment`), which is what makes the source row itself the dedupe key. A form that stays open for successive entries — `/stock/adjust` — must **rotate** that key after each success, or the next entry reads as a replay of the previous one.
