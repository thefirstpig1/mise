...
**Correction, made at L3b (2026-08-17):** this ADR originally said the freshness date comes from the ledger's own `STOCK_COUNT` movements, which required no new column. It is read from **`stock_count_item.counted_at` on a CLOSED count** instead — the count DOCUMENT, which also needs no new column. Closing a count posts **nothing** for a line whose variance is zero (`closeStockCountLogic` skips it), so the ledger cannot see a product that was counted and found exactly right. That is the best-managed stock in the shop, and it would have read "ยังไม่เคยนับ" forever — the freshness line would have been most wrong about precisely the products it had least to warn anyone about. The question the line asks is *"did anyone stand in front of this shelf"*, and only the document knows.
